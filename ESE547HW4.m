...
gamma = 1.4 %% gamma chosen such that time response of Hinf matches RSLQR in accel tracking
    HinfSF.R = [HinfSF.D1'*HinfSF.D1 HinfSF.D1'*HinfSF.D2;...
                HinfSF.D2'*HinfSF.D1 HinfSF.D2'*HinfSF.D2 - gamma^2];
    [P,CL_eig,K] = care(HinfSF.A,HinfSF.B_hat,HinfSF.Q,HinfSF.R,HinfSF.S);
...
% SS model of loop gain at the plant input Lu
     ...
     RSLQRLuFreqResp = allmargin(sys_Lu) 
...
HinfLuFreqResp = allmargin(sys_Lu_act) 
...
taur_act = crosst(e_n1_act,t); % rise time
e_n_act = aze_act - fv_act*ones(size(aze_act)) - 0.05*ones(size(aze_act));
e_n1_act = abs(e_n_act) + e_n_act;
taus_act = crosst(e_n1_act,t); % settling time
...
hold off
legend(['Hinf response ' '63% Tr = ' num2str(taur) ' 95% Ts = ' num2str(taus)],['RSLQR response ' '63% Tr = ' num2str(taur_act) ' 95% Ts = ' num2str(taus_act)]);
...
if(save_plots == 1) saveppt2(plot_file_name); end   
 ...
 % Compute the noise-to-control TF Hinf
 Z = inv(eye(size(Dc1*Dp))-Dc1*Dp);
 Bv = [       Bp*Z*Dc1;
     (Bc1+Bc1*Dp*Z*Dc1)];
 Cv  = [ Z*Dc1*Cp Z*Cc];
 ...
 Z = inv(eye(size(Dc1_act*Dp))-Dc1_act*Dp);
 Bv2 = [       Bp*Z*Dc1_act;
     (Bc1_act+Bc1_act*Dp*Z*Dc1_act)];
 Cv2  = [ Z*Dc1_act*Cp Z*Cc_act];
 Cvv2 = [ Cv2 ; Cv2*Acl_act ];
 Dv2 = Z*Dc1_act;
 Dvv2 = [ Dv2; Cv2*Bv2];
 sys_noise = ss(Acl_act,Bv2,Cvv2,Dvv2);
...
semilogx(w,deledot_Az,'k',w,deledot_q,'b',...
         w,deledot_Az2,'k--',w,deledot_q2,'b--',...
         'LineWidth',2);
legend('Az Hinf','q Hinf',...
       'Az RSlqr','q RSlqr',...
       'Location','Best');
