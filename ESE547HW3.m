...
qq = logspace(-3, 2.5, 500); %these are the varying values of q11, evenly spaced on log scale
...
%% Minimum Singular Value Plots
figure('Name', 'Minimum Singular Values of L')
rdminAll = data(:,2);
srminAll = data(:,3);
wcAll = data(:,4);
hold on 
...
plot(wcAll,srminAll)
xlabel('Wc Hz');
ylabel('min(1+L), min(1+inv(L)');
title('Minimum Singular Values of L');
legend('min RDM Lu', 'min SRM Lu')
hold off

%% Settling Time and Rising Time Plot
figure('Name', 'Settling Time and Rising Time vs Looping Gain Freq')
riseTall = data(:,5);
setTall = data(:,6);
hold on
plot(wcAll,riseTall)
...
title('Settling Time and Rising Time vs Looping Gain Freq');
legend('Rise Time', 'Settling Time')
hold off

%% Overshoot and Undershoot
figure('Name', 'Overshoot% and Undershoot% vs Looping Gain Freq')
overshoot = data(:,8);
undershoot = data(:,7);
hold on
plot(wcAll,overshoot)
plot(wcAll,undershoot)
xlabel('Wc Hz');
ylabel('Percentage');
title('Overshoot% and Undershoot% vs Looping Gain Freq');
legend('Overshoot%', 'Undershoot%')
hold off

%% Max Elevon Degree
figure('Name', 'Max Elevon Deg/g')
Mdegree = data(:,9);
hold on
plot(wcAll,Mdegree/1000)
xlabel('Wc Hz');
ylabel('Max Elevon deg/g');
title('Max Elevon Deg/g');
hold off

%% Max Elevon Rate
figure('Name', 'Max Elevon Rate')
Mrate = data(:,10);
hold on
plot(wcAll,Mrate/1000)
xlabel('Wc Hz');
ylabel('Max Elevon Rate dps/g');
title('Max Elevon Rate');
hold off
...
% [239.197984002024] optimal Q penalty